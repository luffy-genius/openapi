...
def execute():
    request = DigitalHumanRequest(
        ...
        audio_url=AUDIO_URL,
        prompt=PROMPT,
        model=common.env_string('MEDIA_ALIYUN_DIGITAL_HUMAN_MODEL'),
        resolution=RESOLUTION,
        seed=SEED,
        parameters=PARAMETERS,
    )
    with common.media_client(ModelProvider.ALIYUN, 'digital_human') as media:
        return common.complete_result(media, media.avatar.render(request, provider=ModelProvider.ALIYUN))
